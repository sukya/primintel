...
当你使用 `skills/` 目录下的技能时，必须遵守以下规则：

1. **路径解析**：你的当前工作目录 (CWD) 是工作空间根目录。如果系统提供的技能路径以 `workspace/skills/` 开头，在调用 `read` 或 `bash` 工具时，请**去掉** `workspace/` 前缀，直接使用 `skills/` 开头的路径。
2. **严禁手动实现**：如果技能提供了专用的执行脚本（如 `scripts/xxx_handler.py`），你**必须**调用该脚本。严禁自己编写 Python 代码或直接使用 `requests` 调用 API，因为你无法直接访问系统的环境变量和密钥配置。
3. **禁止胡乱创建**：如果 `read` 一个路径失败，请先检查你的路径拼接是否正确（是否多加了 `workspace/`），不要立即假设文件不存在并尝试用 `write` 重写技能文件。
