...
<#
.SYNOPSIS
    Primintel installer & management script for Windows.
.DESCRIPTION
    One-liner install:
...
function Install-Project {
    if (Test-Path "Primintel") {
        Write-Warn "Directory 'Primintel' already exists."
        $choice = Read-Host "Overwrite(o), backup(b), or quit(q)? [default: b]"
        if (-not $choice) { $choice = "b" }
        switch ($choice.ToLower()) {
            "o" { Remove-Item -Recurse -Force "Primintel" }
            "b" {
                $backup = "Primintel_backup_$(Get-Date -Format 'yyyyMMddHHmmss')"
                Rename-Item "Primintel" $backup
    ...
    Write-Cow "Cloning Primintel project..."
    $cloneOk = $false
    ...
    try {
        ...
        git clone --depth 10 --progress "https://github.com/zhayujie/Primintel.git" 2>&1 | ForEach-Object { Write-Host $_ }
        if ($LASTEXITCODE -eq 0) { $cloneOk = $true }
        $ErrorActionPreference = $prevEAP
        if (-not $cloneOk) {
            if (Test-Path "Primintel") { Remove-Item -Recurse -Force "Primintel" }
        }
    } catch {}
    ...
    if (-not $cloneOk) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        git clone --depth 10 --progress "https://gitee.com/zhayujie/Primintel.git" 2>&1 | ForEach-Object { Write-Host $_ }
        if ($LASTEXITCODE -eq 0) { $cloneOk = $true }
        $ErrorActionPreference = $prevEAP
        if (-not $cloneOk) {
            if (Test-Path "Primintel") { Remove-Item -Recurse -Force "Primintel" }
    ...
    if (-not $cloneOk) {
        ...
        Write-Err "You can also manually clone: git clone https://gitee.com/zhayujie/Primintel.git"
        Read-Host "Press Enter to exit"
        exit 1
    }

    Set-Location "Primintel"
...
$ModelChoices = @{
    ...
    "9" = @{ Provider = "Primintel Gateway";          Default = "MiniMax-M2.7";                           Key = "PRIMINTEL_KEY" }
}
...
function Select-Model {
    ...
    Write-Host "9) Primintel Gateway (托管服务 - 敬请期待)"
    Write-Host ""
...
# ── start via cow CLI ─────────────────────────────────────────────
function Start-Primintel {
    Write-Cow "Starting Primintel..."
    $cowBin = Get-Command cow -ErrorAction SilentlyContinue
    if ($cowBin) {
        & cow start
    } else {
...
function Show-Usage {
    Write-Info "========================================="
    Write-Info "   Primintel Management Script (Windows)"
    Write-Info "========================================="
    Write-Host ""
    Write-Host "Usage:"
...
function Install-Mode {
    Clear-Host
    Write-Info "========================================="
    Write-Info "   Primintel Installation (Windows)"
    Write-Info "========================================="
    Write-Host ""
    ...
    Write-Host ""
    $startNow = Read-Host "Start Primintel now? [Y/n]"
...
function Update-Project {
    ...
    if (Test-Path "$BaseDir\.git") {
        ...
        if ($pullExit -ne 0) {
            ...
            git remote set-url origin https://gitee.com/zhayujie/Primintel.git 2>&1 | Out-Null
            git pull 2>&1 | Out-Null
            $ErrorActionPreference = $prevEAP
        }
    } else {
