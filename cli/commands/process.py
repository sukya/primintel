...
def start(foreground, no_logs):
    ...
    pid = _read_pid()
    if pid:
        click.echo(f"Primintel is already running (PID: {pid}).")
        return
    ...
    if foreground:
        click.echo("Starting Primintel in foreground...")
        if _IS_WIN:
        ...
        _write_pid(proc.pid)
        click.echo(click.style(f"✓ Primintel started (PID: {proc.pid})", fg="green"))
...
def stop():
    ...
    pid = _read_pid()
    if not pid:
        click.echo("Primintel is not running.")
        return

    click.echo(f"Stopping Primintel (PID: {pid})...")
    try:
        _kill_pid(pid)
        for _ in range(30):
            time.sleep(0.1)
    ...
    _remove_pid()
    click.echo(click.style("✓ Primintel stopped.", fg="green"))
...
def update(ctx):
    ...
    if _IS_WIN:
        ...
        subprocess.Popen(
            ["cmd.exe", "/c", "start", "Primintel Update", "/wait", bat],
            cwd=root,
        )
...
@click.command()
def status():
    """Show Primintel running status."""
    from cli import __version__
    from cli.utils import load_config_json

    pid = _read_pid()
    if pid:
        click.echo(click.style(f"● Primintel is running (PID: {pid})", fg="green"))
    else:
        click.echo(click.style("● Primintel is not running", fg="red"))
