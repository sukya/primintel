...
class WebChannel(ChatChannel):
    ...
    def startup(self):
        ...
        urls = (
            ...
            '/api/tools/settings', 'ToolSettingsHandler',
            ...
            '/(?:chat/)?outputs/(.*)', 'SkillOutputsHandler',
            '/skills/([^/]+)/outputs/(.*)', 'SkillOutputsHandler',
        )
...
class ConfigHandler:
    ...
    def GET(self):
        ...
        try:
            ...
            title = "Primintel" if use_agent else "AI Assistant"
    ...
    def GET(self):
        ...
        try:
            ...
            for name, cls in tm.tool_classes.items():
                try:
                    instance = cls()
                    # Check if tool has a config file
                    ...
                    tools.append({
                        ...
                        "configurable": os.path.exists(config_path),
                    })
                except Exception as e:
                    logger.warning(f"[WebChannel] Error loading tool {name}: {e}")
                    tools.append({"name": name, "description": "", "configurable": False})
...
class ToolSettingsHandler:
    def GET(self):
        ...
        try:
            cow_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
            # Prioritize workspace skills (custom), then internal skills (builtin)
            config_path = os.path.join(cow_root, "workspace", "skills", tool_name, "resources", "config.json")
            if not os.path.exists(config_path):
                config_path = os.path.join(cow_root, "skills", tool_name, "resources", "config.json")
            
            if not os.path.exists(config_path):
                return json.dumps({"status": "error", "message": "Tool not configurable"})
                
            with open(config_path, 'r', encoding='utf-8') as f:
                config = json.load(f)
            return json.dumps({"status": "success", "config": config})
        except Exception as e:
            return json.dumps({"status": "error", "message": str(e)})
    ...
    def POST(self):
        ...
        try:
            ...
            # Generic merge for most tools, specialized for bing-image-creator
            if tool_name == "bing-image-creator":
                if isinstance(config_updates, dict) and "auth_cookies" in config_updates:
                    current_config["auth_cookies"] = config_updates["auth_cookies"]
                if "default_model" in config_updates:
                    current_config["default_model"] = config_updates["default_model"]
                if "default_aspect" in config_updates:
                    current_config["default_aspect"] = config_updates["default_aspect"]
            else:
                # Generic merge
...
class SkillOutputsHandler:
    """
    Handler for serving static files from skill output directories.
    Supports both internal and workspace skills.
    """
    def GET(self, *args):
        # Depending on the route, args could be (file_path,) or (skill_name, file_path)
        if len(args) == 2:
            skill_name, file_path = args
        else:
            # Default to bing-image-creator if no skill name provided (backwards compatibility)
            skill_name = "bing-image-creator"
            file_path = args[0]

        try:
            cow_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
            
            # Potential locations for skill outputs
            search_paths = [
                os.path.join(cow_root, "skills", skill_name, "outputs", file_path),
                os.path.join(cow_root, "workspace", "skills", skill_name, "outputs", file_path)
            ]
            ...
            if not full_path:
                # Fallback: search in all possible skill output directories (builtin and workspace)
                logger.debug(f"[SkillOutputsHandler] File {file_path} not found in primary paths, trying fallback...")
                for base_dir in ["skills", os.path.join("workspace", "skills")]:
                    skills_base = os.path.join(cow_root, base_dir)
                    if os.path.exists(skills_base):
                        for s_name in os.listdir(skills_base):
                            p = os.path.join(skills_base, s_name, "outputs", file_path)
                            if os.path.isfile(p):
                                full_path = p
                                break
                        if full_path: break
            ...
            if not full_path:
                logger.error(f"[SkillOutputsHandler] File not found in any skill path. Searched: {search_paths}, file_path: {file_path}")
                raise web.notfound()

            # Safety check: ensure the resulting path is within an 'outputs' directory
            if "/outputs/" not in full_path.replace("\\", "/"):
                logger.error(f"[SkillOutputsHandler] Security check failed: {full_path}")
                raise web.notfound()
            ...
            logger.error(f"[SkillOutputsHandler] Error serving skill output: {e}")
            raise web.notfound()
