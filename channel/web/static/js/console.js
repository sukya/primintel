/* =====================================================================
   Primintel Console - Main Application Script
   ===================================================================== */
...
const I18N = {
    zh: {
        ...
        evolution_section_title: '进化技能', evolution_empty_hint: '暂无进化技能',
    ...
    en: {
        ...
        evolution_section_title: 'Evolution Skills', evolution_empty_hint: 'No evolution skills yet',
...
function createMd() {
    ...
    md.renderer.rules.image = function(tokens, idx, options, env, self) {
        tokens[idx].attrPush(['onerror', "this.style.display='none'"]);
...
function normalizeLocalPaths(text) {
    if (!text) return '';
    
    // First, try to catch the specific skills pattern: workspace/skills/<skill>/outputs/<file> or skills/<skill>/outputs/<file>
    // This will match ANY preceding absolute path stuff because of the [^"'\s\),\]\r\n]*?
    text = text.replace(/(?:file:\/\/\/[^"'\s\),\]\r\n]*?|[a-zA-Z]:[^"'\s\),\]\r\n]*?)[\\\/]?(?:workspace[\\\/])?skills[\\\/]([^"'\s\),\]\r\n]+)[\\\/]outputs[\\\/]([^"'\s\),\]\r\n]+)/gi, '/skills/$1/outputs/$2');
    
    // Then try generic outputs for paths not in skills/
    text = text.replace(/(?:file:\/\/\/[^"'\s\),\]\r\n]*?|[a-zA-Z]:[^"'\s\),\]\r\n]*?)[\\\/]?outputs[\\\/]([^"'\s\),\]\r\n]+)/gi, '/outputs/$1');
    
    return text;
}

function extractMediaHtml(text) {
    if (!text) return '';
    const mediaRegex = /(?:\/?outputs\/|\/?assets\/|\/?uploads\/|\/?skills\/)[^"'\s\),\]\r\n\\]+/gi;
    const matches = text.match(mediaRegex);
    if (!matches) return '';

    let galleryHtml = '<div class="media-gallery flex flex-wrap gap-2 mt-2">';
    let found = false;
    
    [...new Set(matches)].forEach(match => {
        const url = match.replace(/[),.\s'"]+$/, ''); 
        const bareUrl = url.split('?')[0];
        
        if (IMAGE_EXT_RE.test(bareUrl)) {
            galleryHtml += `<div class="w-48 h-48 rounded-lg overflow-hidden border border-slate-200 dark:border-white/10 shadow-sm hover:shadow-md transition-shadow cursor-pointer" onclick="_openImageLightbox('${escapeHtml(_toWebUrl(url))}')">
                <img src="${escapeHtml(_toWebUrl(url))}" class="w-full h-full object-cover" onerror="this.parentElement.style.display='none'">
            </div>`;
            found = true;
        } else if (VIDEO_EXT_RE.test(bareUrl)) {
            galleryHtml += `<div class="w-64 h-48 rounded-lg overflow-hidden border border-slate-200 dark:border-white/10 shadow-sm relative group bg-black">
                <video src="${escapeHtml(_toWebUrl(url))}" class="w-full h-full object-contain"></video>
                <div class="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/40 transition-colors cursor-pointer" onclick="window.open('${escapeHtml(_toWebUrl(url))}', '_blank')">
    ...
    galleryHtml += '</div>';
    return found ? galleryHtml : '';
}

function _toWebUrl(url) {
    if (!url) return '';
    // Skip already managed paths or absolute URLs
    if (/^https?:\/\//i.test(url) || /^\/(?:api|skills|static|assets|uploads|outputs)\//.test(url)) {
        return url;
    }
    // Handle Windows absolute paths (e.g., C:\... or D:/...)
    if (/^[a-zA-Z]:[\\\/]/.test(url)) {
    ...
    if (/^file:\/\/\//i.test(url)) {
        ...
        // On Windows, file:///D:/... becomes D:/... after replace.
        // On Unix, file:///home/... becomes home/... which needs a leading slash.
        if (!/^[a-zA-Z]:/.test(path) && !path.startsWith('/')) {
            path = '/' + path;
        }
        return '/api/file?path=' + encodeURIComponent(path);
    }
    // If it's a relative path starting with skills/ or outputs/, add leading slash
    if (/^(?:skills|outputs|assets|uploads)\//.test(url)) {
        return '/' + url;
    }
    return url;
}
...
function injectVideoPlayers(html) {
    ...
    return step1.split(/(<[^>]+>)/).map((chunk, idx) => {
        ...
        // Match http(s) URLs OR local root paths (/outputs/, /assets/, /uploads/)
        return chunk.replace(/(?:https?:\/\/|\/outputs\/|\/assets\/|\/uploads\/|\/skills\/)[^"'\s\),\]\r\n\\]+/gi, (url) => {
            const bare = url.replace(/[),.\s'"]+$/, '');  // strip trailing punctuation or quotes
...
function injectImagePreviews(html) {
    ...
    const step1 = html.replace(
        /<a\s+href="(https?:\/\/[^"]+|\/outputs\/[^"]+|\/assets\/[^"]+|\/uploads\/[^"]+|\/skills\/[^"]+)"[^>]*>[^<]*<\/a>/gi,
...
function renderMarkdown(text) {
    if (!text) return '';
    try {
        const gallery = extractMediaHtml(text);
        text = normalizeLocalPaths(text);
...
let appConfig = { use_agent: false, title: 'Primintel', subtitle: '', providers: {}, api_bases: {} };
...
fetch('/config').then(r => r.json()).then(data => {
    if (data.status === 'success') {
        appConfig = data;
        const title = data.title || 'Primintel';
...
function startSSE(requestId, loadingEl, timestamp, titleInfo) {
    ...
    function ensureBotEl() {
        ...
        botEl.innerHTML = `
            <img src="assets/logo.jpg" alt="Primintel" class="w-8 h-8 rounded-lg flex-shrink-0">
...
function createUserMessageEl(content, timestamp, attachments) {
    ...
    if (attachments && attachments.length > 0) {
        const items = attachments.map(a => {
            const previewUrl = normalizeLocalPaths(a.preview_url || ''); // Normalize history paths
            if (a.file_type === 'image') {
                return `<img src="${previewUrl}" alt="${escapeHtml(a.file_name)}" class="user-msg-image">`;
            }
    ...
    const textHtml = textRaw ? renderMarkdown(textRaw) : '';
    const textMediaHtml = extractMediaHtml(normalizeLocalPaths(textRaw)); // Extra gallery for text content
    
    el.innerHTML = `
        <div class="max-w-[75%] sm:max-w-[60%]">
            <div class="bg-primary-400 text-white rounded-2xl px-4 py-2.5 text-sm leading-relaxed msg-content user-bubble">
                ${attachHtml}${textHtml}${textMediaHtml}
            </div>
...
function renderToolCallsHtml(toolCalls) {
    ...
    return toolCalls.map(tc => {
        ...
        let resultRaw = tc.result ? String(tc.result) : '';
        resultRaw = normalizeLocalPaths(resultRaw); // Normalize paths in history data
        
        const resultStr = escapeHtml(resultRaw);
        const mediaHtml = extractMediaHtml(resultRaw); // Extract images from normalized paths
...
function renderStepsHtml(steps) {
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        if (step.type === 'thinking') {
            ...
            let resultRaw = step.result ? String(step.result) : '';
            resultRaw = normalizeLocalPaths(resultRaw); // Normalize paths in history data
            
            const resultStr = escapeHtml(resultRaw);
            const mediaHtml = extractMediaHtml(resultRaw); // Extract gallery from results
            html += `
<div class="agent-step agent-tool-step">
    <div class="tool-header" onclick="this.parentElement.classList.toggle('expanded')">
        <i class="fas fa-check text-primary-400 flex-shrink-0 tool-icon"></i>
    ...
    <div class="tool-detail">
        ...
        <div class="tool-detail-section tool-output-section">
            ...
            const sentFileMediaHtml = _renderSentFileFromToolResult(step);
            if (sentFileMediaHtml) html += sentFileMediaHtml;
        }
    }
...
function newChat() {
    ...
    ws.innerHTML = `
        <img src="assets/logo.jpg" alt="Primintel" class="w-16 h-16 rounded-2xl mb-6 shadow-lg shadow-primary-500/20">
        <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-3">${appConfig.title || 'Primintel'}</h1>
            ...
            card.innerHTML = `
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                        ...
                        ${tool.configurable ? `
                        <button onclick="openToolSettings('${escapeHtml(tool.name)}')" 
                                class="ml-auto p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 text-slate-400 hover:text-primary-500 transition-colors"
                                title="${currentLang === 'zh' ? '配置' : 'Settings'}">
...
function loadSkillsSection() {
    ...
    const evoWrap = document.getElementById('evolution-section-wrap');
    const evoList = document.getElementById('evolution-list');
    const evoBadge = document.getElementById('evolution-count-badge');
    const evoEmpty = document.getElementById('evolution-empty');
    ...
    fetch('/api/skills').then(r => r.json()).then(data => {
        ...
        const evolutionSkills = allSkills.filter(sk => sk.category === 'evolution');
        const standardSkills = allSkills.filter(sk => sk.category !== 'evolution');
        ...
        if (standardSkills.length === 0) {
            ...
            badge.textContent = standardSkills.length;
            badge.classList.remove('hidden');
            emptyEl.classList.add('hidden');
            listEl.innerHTML = '';
        ...
        // Handle Evolution Skills
        if (evolutionSkills.length > 0) {
            evoWrap.classList.remove('hidden');
            evoBadge.textContent = evolutionSkills.length;
            evoEmpty.classList.add('hidden');
            evoList.innerHTML = '';
            ...
            evoList.classList.remove('hidden');
        } else {
            evoWrap.classList.add('hidden');
        }
    }).catch(() => {});
}
...
function renderSkillCard(card, sk) {
    ...
    const isEvo = sk.category === 'evolution';
    const iconColor = enabled ? (isEvo ? 'text-indigo-400' : 'text-primary-400') : 'text-slate-300 dark:text-slate-600';
    const trackClass = enabled
        ? (isEvo ? 'bg-indigo-400' : 'bg-primary-400')
        : 'bg-slate-200 dark:bg-slate-700';
    const bgClass = isEvo ? 'bg-indigo-50 dark:bg-indigo-900/20' : 'bg-amber-50 dark:bg-amber-900/20';
    const iconClass = isEvo ? 'fa-dna' : 'fa-bolt';
    const thumbTranslate = enabled ? 'translate-x-3' : 'translate-x-0.5';
    card.innerHTML = `
        <div class="w-9 h-9 rounded-lg ${bgClass} flex items-center justify-center flex-shrink-0">
        ...
        <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1">
                ...
                ${sk.configurable ? `
                <button onclick="openToolSettings('${escapeHtml(sk.name)}')" 
                        class="p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 text-slate-400 hover:text-primary-500 transition-colors"
                        title="${currentLang === 'zh' ? '配置' : 'Settings'}">
...
// Tool Settings
// =====================================================================
let currentConfiguringTool = null;
let currentConfigData = null;

function openToolSettings(name) {
    currentConfiguringTool = name;
    ...
    const saveBtn = document.getElementById('tool-settings-save');
    
    title.textContent = name === 'bing-image-creator' ? (currentLang === 'zh' ? '必应生图配置' : 'Bing Image Settings') : name;
    body.innerHTML = `<div class="flex items-center justify-center py-10 text-slate-400"><i class="fas fa-spinner fa-spin mr-2"></i><span>${currentLang === 'zh' ? '正在加载配置...' : 'Loading config...'}</span></div>`;
    saveBtn.disabled = true;
    modal.classList.remove('hidden');

    fetch(`/api/tools/settings?name=${encodeURIComponent(name)}`)
        .then(r => r.json())
        .then(data => {
            if (data.status === 'success') {
                currentConfigData = data.config;
                renderToolConfig(name, data.config);
                saveBtn.disabled = false;
            } else {
                body.innerHTML = `<p class="text-sm text-red-500 py-4">${escapeHtml(data.message)}</p>`;
            }
        })
        .catch(err => {
            body.innerHTML = `<p class="text-sm text-red-500 py-4">Failed to load config</p>`;
        });
}

function closeToolSettings() {
    document.getElementById('tool-settings-modal').classList.add('hidden');
    currentConfiguringTool = null;
    currentConfigData = null;
}

window.updateBingAspectOptions = function(defaultVal) {
    const modelSelect = document.getElementById('config-default-model');
    const aspectSelect = document.getElementById('config-default-aspect');
    if (!modelSelect || !aspectSelect) return;
    
    const model = modelSelect.value;
    const currentVal = defaultVal || aspectSelect.value;
    
    aspectSelect.innerHTML = '';
    
    const isZh = typeof currentLang !== 'undefined' && currentLang === 'zh';
    const optS = isZh ? '正方形 (1:1)' : 'Square (1:1)';
    let optL, optP;
    
    if (model === 'dalle3') {
        optL = isZh ? '横向 (7:4)' : 'Landscape (7:4)';
        optP = isZh ? '竖向 (4:7)' : 'Portrait (4:7)';
    } else {
        optL = isZh ? '横向 (3:2)' : 'Landscape (3:2)';
        optP = isZh ? '竖向 (2:3)' : 'Portrait (2:3)';
    }
    
    const opts = [
        {value: 'square', text: optS},
        {value: 'landscape', text: optL},
        {value: 'portrait', text: optP}
    ];
    ...
    opts.forEach(o => {
        ...
        if (o.value === currentVal) option.selected = true;
        aspectSelect.appendChild(option);
    });
};

function renderToolConfig(name, config) {
    const body = document.getElementById('tool-settings-body');
    if (name === 'bing-image-creator') {
        const cookies = config.auth_cookies || [];
        const d_model = config.default_model || "dalle3";
        const d_aspect = config.default_aspect || "square";

        const labelModel = currentLang === 'zh' ? '默认模型' : 'Default Model';
        const labelAspect = currentLang === 'zh' ? '默认图片比例' : 'Default Aspect Ratio';

        const optSquare = currentLang === 'zh' ? '正方形 (1:1)' : 'Square (1:1)';
        const optLandscape = currentLang === 'zh' ? '横向 (Horizontal)' : 'Landscape (Horizontal)';
        const optPortrait = currentLang === 'zh' ? '竖向 (Vertical)' : 'Portrait (Vertical)';

        let html = `<div class="space-y-4">
            <p class="text-xs text-slate-500 dark:text-slate-400 mb-2">
                ${currentLang === 'zh' ? '在此处管理您的默认生图偏好和账户 Cookie。' : 'Manage your default generation preferences and account Cookies here.'}
            </p>
            
            <div class="grid grid-cols-2 gap-3 mb-4">
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">${labelModel}</label>
                    <select id="config-default-model" onchange="updateBingAspectOptions()" class="px-3 py-2 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 focus:outline-none focus:border-primary-500 transition-colors">
                        <option value="dalle3" ${d_model === 'dalle3' ? 'selected' : ''}>DALL-E 3</option>
                        <option value="gpt4o" ${d_model === 'gpt4o' ? 'selected' : ''}>GPT-4o</option>
                        <option value="mai1" ${d_model === 'mai1' ? 'selected' : ''}>MAI-Image-1</option>
                    </select>
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">${labelAspect}</label>
                    <select id="config-default-aspect" class="px-3 py-2 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 focus:outline-none focus:border-primary-500 transition-colors">
                        <!-- Options generated dynamically -->
                    </select>
                </div>
            </div>
            
            <div id="bing-cookie-list" class="space-y-3">
        `;
        
        cookies.forEach((c, idx) => {
            const statusClass = c.status === 'active' ? 'bg-emerald-500/10 text-emerald-500' : 
                               (c.status === 'expired' ? 'bg-rose-500/10 text-rose-500' : 'bg-amber-500/10 text-amber-500');
            const statusText = currentLang === 'zh' ? 
                               (c.status === 'active' ? '活跃' : (c.status === 'expired' ? '已过期' : '已耗尽')) : 
                               (c.status.charAt(0).toUpperCase() + c.status.slice(1));
                               
            html += `
                <div class="p-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 relative group">
                    <div class="flex items-center justify-between mb-2">
                        <div class="flex items-center gap-2">
                            <span class="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Cookie #${idx + 1}</span>
                            <span class="px-1.5 py-0.5 rounded text-[9px] font-bold ${statusClass}">${statusText}</span>
                            ${c.status !== 'active' ? `<button onclick="reactivateBingCookie(${idx})" class="text-[9px] text-primary-500 hover:underline ml-1">${currentLang === 'zh' ? '点击重新激活' : 'Reactivate'}</button>` : ''}
                        </div>
                        <button onclick="removeBingCookie(${idx})" class="text-slate-300 hover:text-red-500 transition-colors">
                            <i class="fas fa-trash-can text-[10px]"></i>
                        </button>
                    </div>
                    <textarea class="bing-cookie-input w-full px-3 py-2 text-xs font-mono rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 focus:outline-none focus:border-primary-500 transition-colors"
                              rows="3" placeholder="Paste _U cookie here...">${escapeHtml(c.cookie_U)}</textarea>
                    ${c.error_reason ? `<p class="mt-1 text-[9px] text-rose-400/80 italic break-words">Error: ${escapeHtml(c.error_reason)}</p>` : ''}
                </div>
            `;
        });
        
        html += `</div>
            <button onclick="addBingCookie()" class="w-full py-2 border-2 border-dashed border-slate-200 dark:border-white/10 rounded-xl text-slate-400 hover:text-primary-500 hover:border-primary-300 dark:hover:border-primary-800 transition-all text-xs font-medium">
                <i class="fas fa-plus mr-1"></i> ${currentLang === 'zh' ? '添加 Cookie' : 'Add Cookie'}
            </button>
        </div>`;
        body.innerHTML = html;
        setTimeout(() => updateBingAspectOptions(d_aspect), 0);
    } else {
        // Generic rendering for other tools
        let html = '<div class="space-y-4">';
        const schema = config._schema || {};
        Object.entries(config).forEach(([key, value]) => {
            if (key === '_schema') return; // Skip metadata
            
            let itemSchema = schema[key] || {};
            if (itemSchema.type === 'conditional' && itemSchema.depends_on) {
                const depKey = itemSchema.depends_on;
                const depValue = config[depKey];
                itemSchema = (itemSchema.cases && itemSchema.cases[depValue]) || (itemSchema.cases && itemSchema.cases.default) || {};
            }

            const isSecret = key.toLowerCase().includes('key') || key.toLowerCase().includes('token') || key.toLowerCase().includes('secret');
            
            // Basic label mapping for common keys
            let label = key;
            if (currentLang === 'zh') {
                if (key === 'api_key') label = 'API Key';
                else if (key === 'model_name') label = '模型名称 (Model Name)';
                else if (key === 'size') label = '分辨率 (Size)';
            }
            
            const onchange = `onConfigFieldChange('${escapeHtml(key)}', this.value)`;
            
            html += `
                <div class="flex flex-col gap-1.5">
                    <div class="flex items-center justify-between">
                        <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">${escapeHtml(label)}</label>
                        ${itemSchema.hint ? `<span class="text-[10px] text-slate-400 italic">${escapeHtml(itemSchema.hint)}</span>` : ''}
                    </div>`;
            
            if (itemSchema.type === 'select' && itemSchema.options) {
                html += `<select data-key="${escapeHtml(key)}" onchange="${onchange}"
                                 class="generic-config-input px-3 py-2 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 focus:outline-none focus:border-primary-500 transition-colors">`;
                itemSchema.options.forEach(opt => {
                    const optVal = typeof opt === 'object' ? opt.value : opt;
                    const optLabel = typeof opt === 'object' ? (opt.label || opt.value) : opt;
                    html += `<option value="${escapeHtml(String(optVal))}" ${value == optVal ? 'selected' : ''}>${escapeHtml(String(optLabel))}</option>`;
                });
                html += `</select>`;
                ...
                html += `
                    <div class="flex items-center mt-1">
                        <input type="checkbox" data-key="${escapeHtml(key)}" 
                               onchange="onConfigFieldChange('${escapeHtml(key)}', this.checked)"
                               ...
                               ${value ? 'checked' : ''}>
                        <span class="ml-2 text-xs text-slate-400">${itemSchema.hint || ''}</span>
                    </div>`;
            } else {
                html += `<input type="${isSecret ? 'password' : 'text'}" data-key="${escapeHtml(key)}" onchange="${onchange}"
                               placeholder="${escapeHtml(itemSchema.placeholder || '')}"
                               class="generic-config-input w-full px-3 py-2 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 focus:outline-none focus:border-primary-500 transition-colors"
...
window.onConfigFieldChange = function(key, value) {
    if (!currentConfigData) return;
    currentConfigData[key] = value;
    renderToolConfig(currentConfiguringTool, currentConfigData);
};

function addBingCookie() {
    const list = document.getElementById('bing-cookie-list');
    const idx = list.children.length;
    const div = document.createElement('div');
    div.className = 'p-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 relative group';
    div.innerHTML = `
        <div class="flex items-center justify-between mb-2">
            <span class="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Cookie #${idx + 1}</span>
            <button onclick="this.parentElement.parentElement.remove()" class="text-slate-300 hover:text-red-500 transition-colors">
                <i class="fas fa-trash-can text-[10px]"></i>
            </button>
        </div>
        <textarea class="bing-cookie-input w-full px-3 py-2 text-xs font-mono rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 focus:outline-none focus:border-primary-500 transition-colors"
                  rows="3" placeholder="Paste _U cookie here..."></textarea>
    `;
    list.appendChild(div);
}

function removeBingCookie(idx) {
    const list = document.getElementById('bing-cookie-list');
...
function saveToolSettings() {
    if (!currentConfiguringTool) return;
    
    const btn = document.getElementById('tool-settings-save');
    const status = document.getElementById('tool-settings-status');
    btn.disabled = true;
    
    let config = {};
    if (currentConfiguringTool === 'bing-image-creator') {
        const inputs = document.querySelectorAll('.bing-cookie-input');
        const cookies = [];
        inputs.forEach(inp => {
        ...
        config = { 
            ...
            default_model: document.getElementById('config-default-model').value,
            default_aspect: document.getElementById('config-default-aspect').value
        };
    } else {
        // Generic collector for other tools
        const inputs = document.querySelectorAll('.generic-config-input');
        inputs.forEach(inp => {
    ...
    fetch('/api/tools/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: currentConfiguringTool, config })
    })
    .then(r => r.json())
    .then(data => {
        if (data.status === 'success') {
            status.textContent = currentLang === 'zh' ? '已保存' : 'Saved';
...
window.reactivateBingCookie = function(idx) {
    const inputs = document.querySelectorAll('.bing-cookie-input');
    if (!inputs[idx]) return;
    
    // We update the UI internally for visual feedback then trigger a save
    const card = inputs[idx].closest('.p-3');
    const badge = card.querySelector('span.px-1\\.5');
    if (badge) {
        badge.className = 'px-1.5 py-0.5 rounded text-[9px] font-bold bg-emerald-500/10 text-emerald-500';
        badge.textContent = currentLang === 'zh' ? '待保存' : 'Pending';
    }
    
    // Force a save to update backend
    saveToolSettings();
}
...
const WECOM_BOT_SOURCE = 'primintel';
...
function initApp() {
    ...
    fetch('/api/version').then(r => r.json()).then(data => {
        ...
        document.getElementById('sidebar-version').textContent = `Primintel ${APP_VERSION}`;
    }).catch(() => {
        document.getElementById('sidebar-version').textContent = 'Primintel';
    });
    chatInput.focus();
}
