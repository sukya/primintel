...
class ChatChannel(Channel):
    ...
    def _extract_and_send_images(self, reply: Reply, context: Context):
        ...
        # Clean the text content by removing the extracted media tags to prevent showing raw links in IM channels
        clean_content = content
        for pattern, _ in patterns:
            # We don't want to strip pure URLs if they are the only content, 
            # but for markdown and tags it's better to hide them.
        ...
        if media_items:
            ...
            # Send cleaned text first if anything remains
            if clean_content:
                reply.content = clean_content
                logger.info(f"[chat_channel] Sending text content before media: {clean_content[:100]}...")
                self._send(reply, context)
            else:
                logger.info("[chat_channel] No text content left after stripping media tags")
                
            logger.info(f"[chat_channel] Now sending {len(media_items)} media item(s)")
            ...
            for i, (url, media_type) in enumerate(media_items):
                try:
                    ...
                    if url.startswith(('http://', 'https://')):
                        ...
                        local_path = url
                        if media_type == 'video':
                            media_reply = Reply(ReplyType.FILE, f"file://{local_path}")
                            media_reply.file_name = os.path.basename(local_path)
                        else:
                            media_reply = Reply(ReplyType.IMAGE_URL, f"file://{local_path}")
                    else:
                        # Path may be relative to the project root (e.g. /skills/xxx/outputs/img.jpg)
                        # Try resolving against the current working directory
                        cwd = os.getcwd()
                        # Strip leading slash so os.path.join works correctly
                        stripped = url.lstrip('/')
                        alt_path = os.path.join(cwd, stripped)
                        workspace_path = os.path.join(cwd, "workspace", stripped)
