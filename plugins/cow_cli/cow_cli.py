...
class CowCliPlugin(Plugin):
    ...
    def _cmd_version(self, args: str, e_context, **_) -> str:
        return f"Primintel v{__version__}"
    ...
    def _cmd_install_browser(self, args: str, e_context, **_) -> str:
        ...
        return "✅ 安装流程已结束。请重启 Primintel 后使用 browser 工具（进度见上方消息）。"
    ...
    def _skill_list_remote(self, page: int = 1) -> str:
        ...
        lines.append("🌐 https://skills.primintel.ai  在线浏览全部技能")
        return "\n".join(lines)
    ...
    def _cmd_evolve(self, args: str, e_context, session_id: str = "", **_) -> str:
        """
        /evolve — Manually trigger skill distillation on the current session.

        Runs synchronously so the user gets immediate feedback.
        Requires enable_skill_distillation=true OR the user explicitly
        enables it via /config enable_skill_distillation true first.
        """
        from config import conf
        session_id = self._get_session_id(e_context, fallback=session_id)
        agent = self._get_agent(session_id)
        ...
        if not agent:
            return (
                "⚠️ Agent 尚未初始化，请先发送一条对话消息再执行 /evolve\n\n"
                "💡 若要启用自动进化，可执行: /config enable_skill_distillation true"
            )
        ...
        if not messages:
            return "⚠️ 当前对话为空，无法提炼技能。请先与 Agent 完成一轮多步骤对话。"

        if not agent.workspace_dir:
            return "⚠️ workspace_dir 未配置，无法保存技能文件。"

        # Temporarily override the threshold so /evolve always attempts distillation
        # regardless of the current distillation_min_tools setting
        try:
            from agent.skills.distill import (
                evaluate_trajectory, distill_skill_sync, _build_trajectory_snapshot
            )
        except ImportError as e:
            return f"❌ 进化模块加载失败: {e}"

        tool_count = len(self._count_distinct_tools(messages))
        threshold = int(conf().get("distillation_min_tools", 2))

        if tool_count < threshold:
            return (
                f"⚠️ 当前对话的工具调用数（{tool_count}）低于提炼阈值（{threshold}）\n\n"
                f"💡 请先完成一轮包含多步骤工具调用的任务，再运行 /evolve\n"
                f"💡 或将阈值调低: /config distillation_min_tools 1"
            )

        try:
            created_path = distill_skill_sync(
                messages=messages,
                model=agent.model,
        ...
        if created_path:
            skill_name = os.path.basename(created_path)
            return (
                f"✅ 技能进化完成！\n\n"
                f"📌 新技能: {skill_name}\n"
                f"📁 路径: {created_path}\n\n"
                f"💡 使用 /skill list 查看全部技能\n"
                f"💡 技能已在下次对话时自动生效"
            )
        else:
            return (
                "🤔 本次对话未识别出可复用的工作流\n\n"
                "这不影响任何功能。AI 判断本次任务属于一次性场景，不值得单独提炼为技能。\n"
                "💡 尝试让 Agent 完成一个具有明确多步骤流程的复杂任务后再试。"
            )

    @staticmethod
    def _count_distinct_tools(messages: list) -> list:
        """Count unique tool names across all assistant messages."""
        seen = []
        for msg in messages:
