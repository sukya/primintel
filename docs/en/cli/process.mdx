...
description: Manage Primintel process lifecycle with cow commands
---

Process management commands control the Primintel background process. These commands are only available in the terminal.

## start

Start the Primintel service. Runs as a background daemon by default and automatically tails logs.
...
Stop the running Primintel service.
...
Restart the Primintel service (stop then start).
...
Check Primintel service status, including process info, version, and current model/channel configuration.
