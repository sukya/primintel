...
# Clone project
clone_project() {
    echo -e "${GREEN}🔍 Cloning Primintel project...${NC}"

    if [ -d "Primintel" ]; then
        echo -e "${YELLOW}⚠️  Directory 'Primintel' already exists.${NC}"
        read -p "Choose action: overwrite(o), backup(b), or quit(q)? [press Enter for default: b]: " choice
        choice=${choice:-b}
        case "$choice" in
            o|O)
                echo -e "${YELLOW}🗑️  Overwriting 'Primintel' directory...${NC}"
                rm -rf Primintel
                ;;
            b|B)
                backup_dir="Primintel_backup_$(date +%s)"
                echo -e "${YELLOW}🔀 Backing up to '$backup_dir'...${NC}"
                mv Primintel "$backup_dir"
                ;;
            q|Q)
    ...
    if ! command -v git &> /dev/null; then
        ...
        local zip_url="https://gitee.com/zhayujie/Primintel/repository/archive/master.zip"
        if command -v wget &> /dev/null; then
            wget "$zip_url" -O Primintel.zip
        elif command -v curl &> /dev/null; then
            curl -L "$zip_url" -o Primintel.zip
        else
            echo -e "${RED}❌ Cannot download project. Please install Git, wget, or curl.${NC}"
            exit 1
        fi
        unzip Primintel.zip
        mv Primintel-master Primintel
        rm Primintel.zip
    else
        ...
        if curl -sI --connect-timeout 5 --max-time 10 https://github.com > /dev/null 2>&1; then
            ...
            _timeout 60 git clone --depth 10 --progress https://github.com/zhayujie/Primintel.git && clone_ok=true
        fi
        if [ "$clone_ok" = false ]; then
            echo -e "${YELLOW}⚠️  GitHub clone failed or timed out, switching to Gitee mirror...${NC}"
            _timeout 30 git clone --depth 10 --progress https://gitee.com/zhayujie/Primintel.git && clone_ok=true
        fi
        if [ "$clone_ok" = false ]; then
    ...
    cd Primintel || { echo -e "${RED}❌ Failed to enter project directory.${NC}"; exit 1; }
    export BASE_DIR=$(pwd)
...
select_model() {
    ...
    echo -e "${YELLOW}10) Primintel Gateway (托管服务 - 敬请期待)${NC}"
    echo ""
...
configure_model() {
    case "$model_choice" in
        ...
        10)
            read_model_config "Primintel Gateway" "MiniMax-M2.7" "PRIMINTEL_KEY"
            USE_LINKAI="true"
            ;;
    esac
}
...
start_project() {
    echo ""
    echo -e "${GREEN}${EMOJI_ROCKET} Starting Primintel...${NC}"
    sleep 1
    ...
    if $USE_COW; then
        ...
        if [[ "$OS_TYPE" == "Linux" ]]; then
            ...
            echo -e "${GREEN}${EMOJI_COW} Primintel started on Linux (using $PYTHON_CMD)${NC}"
        elif [[ "$OS_TYPE" == "Darwin" ]]; then
            nohup $PYTHON_CMD "${BASE_DIR}/app.py" > "${BASE_DIR}/nohup.out" 2>&1 &
            echo -e "${GREEN}${EMOJI_COW} Primintel started on macOS (using $PYTHON_CMD)${NC}"
        else
    ...
    echo ""
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${GREEN}${EMOJI_CHECK} Primintel is now running in background!${NC}"
...
show_usage() {
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_COW} Primintel Management Script${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
...
cmd_start() {
    ...
    if has_cow; then
        ...
        if is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} Primintel is already running (PID: $(get_pid))${NC}"
...
cmd_stop() {
    if has_cow; then
        ...
        echo -e "${GREEN}${EMOJI_STOP} Stopping Primintel...${NC}"

        if ! is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} Primintel is not running${NC}"
            return
        fi
        ...
        echo -e "${GREEN}${EMOJI_CHECK} Primintel stopped${NC}"
    fi
}
...
cmd_status() {
    if has_cow; then
        ...
        echo -e "${CYAN}${BOLD}=========================================${NC}"
        echo -e "${CYAN}${BOLD}   ${EMOJI_COW} Primintel Status${NC}"
        echo -e "${CYAN}${BOLD}=========================================${NC}"
...
# Reconfigure
cmd_config() {
    echo -e "${YELLOW}${EMOJI_WRENCH} Reconfiguring Primintel...${NC}"
...
# Update project
cmd_update() {
    echo -e "${GREEN}${EMOJI_WRENCH} Updating Primintel...${NC}"
    cd "${BASE_DIR}"
    ...
    if [ -d .git ]; then
        ...
        if git pull; then
            ...
            git remote set-url origin https://gitee.com/zhayujie/Primintel.git
            if git pull; then
                pull_ok=true
            else
...
install_mode() {
    clear
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_COW} Primintel Installation${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
    sleep 1
    ...
    echo ""
    read -p "Start Primintel now? [Y/n]: " start_now
