...
class Agent:
    ...
    def run_stream(self, user_message: str, on_event=None, clear_history: bool = False, skill_filter=None) -> str:
        ...
        # --- Hermes-style Skill Distillation ---
        # Silently analyze the completed trajectory and auto-generate a reusable
        # Skill if the conversation involved a complex, multi-tool workflow.
        if conf().get("enable_skill_distillation", False) and self.workspace_dir:
            try:
                from agent.skills.distill import distill_skill_async
                distill_skill_async(
                ...
                logger.debug(f"[Agent] Skill distillation skipped: {_distill_err}")
