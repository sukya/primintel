...
class BrowserTool(BaseTool):
    ...
    def _do_screenshot(self, args: Dict[str, Any]) -> ToolResult:
        ...
        # Get absolute path from service
        absolute_path = self._get_service().screenshot(full_page=full_page, cwd=self.cwd)
        
        # Convert to relative path for better cross-tool compatibility
        try:
            relative_path = os.path.relpath(absolute_path, self.cwd)
            # Ensure use of forward slashes even on Windows for tool-to-tool consistency
        ...
        return ToolResult.success(
            ...
            f"Format: [FILE_PATH] {relative_path} [/FILE_PATH]\n"
            f"Note: You can pass this exact [FILE_PATH] content to the 'send' tool to show it to the user."
        )
