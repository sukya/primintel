...
class ChatService:
    ...
    def run(self, query: str, session_id: str, send_chunk_fn: Callable[[dict], None],
        ...
        # Mirror the same hook that agent.run_stream() applies, because ChatService
        # calls the executor directly and bypasses that path entirely.
        from config import conf as _conf
        if _conf().get("enable_skill_distillation", False) and agent.workspace_dir:
            try:
                ...
                logger.debug(f"[ChatService] Skill distillation skipped: {_de}")
